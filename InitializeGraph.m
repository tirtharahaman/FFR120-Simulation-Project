...
function [agentPlot, foodPlot] = InitializeGraph(agentLattice, nAgents, nFoodTiles)
    ...
    agentPlot = plot(horisontalAxis,ones(2,1)*nAgents(1)/maxPopulation,'Color',[0.05 0.05 0.9]);
    hold on
    foodPlot = plot(horisontalAxis,ones(2,1)*nFoodTiles(1)/maxPopulation,'Color',[0.05 0.9 0.05]);
